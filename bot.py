...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8854058497:AAEfrNAOynipf_4CT9k23ftWESL5wmhCU-A")
...
async def bot_loop(context, uid, d):
    await context.bot.send_message(uid, "🚀 Bot Started! Scanning P2P...")
    while d.get("running"):
        try:
            await scan_and_buy(context, uid, d)
            ...
            async with session.post(url, json=payload, headers=headers) as r:
                ...
                for ad in ads:
                    ...
                    if buy_amt < min_amt or buy_amt > max_amt:
                        continue
                    ok = await place_order(session, d, adv_no, buy_amt, price)
                    if ok:
                        diff = price - d["target"]
                        msg = (f"🟢 SUCCESS!\n\n"
                               f"💰 Diff: {diff:.1f}\n"
...
async def place_order(session, d, adv_no, amount, price):
    try:
        ts = int(time.time() * 1000)
        payload = {"advNo": adv_no, "tradeType": "BUY",
                   "asset": d["coin"], "fiatUnit": d["fiat"],
                   "amount": str(amount), "price": str(price),
                   "timestamp": ts}
        qs = str(payload).replace("'", '"').replace(" ", "")
        sig = hmac.new(d["secret"].encode(), qs.encode(), hashlib.sha256).hexdigest()
        ...
        async with session.post(url, json=payload, headers=headers) as r:
            res = await r.json()
            return res.get("success") or res.get("code") == "000000"
    except Exception as e:
...
async def cb_stats(update, context):
    ...
    kb = [[InlineKeyboardButton("📋 Order History", callback_data="history")],
...
async def cb_settings(update, context):
    ...
    kb = [
        ...
        [InlineKeyboardButton(f"API Key [{api_ok}] Secret [{sec_ok}]", callback_data="s_api")],
        [InlineKeyboardButton("◀️ Back", callback_data="menu")],
    ]
    await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb))
    return SETTINGS
...
async def s_fiat(update, context):
    ...
    kb = [[InlineKeyboardButton("MMK", callback_data="f_MMK"),
           ...
           InlineKeyboardButton("THB", callback_data="f_THB")],
...
async def s_pay(update, context):
    ...
    await q.edit_message_text("💳 Payment Methods ရွေးပါ:", reply_markup=InlineKeyboardMarkup(kb))
    return S_PAY
...
async def s_coin(update, context):
    ...
    kb = [[InlineKeyboardButton("USDT", callback_data="c_USDT"),
           ...
           InlineKeyboardButton("ETH", callback_data="c_ETH")],
          [InlineKeyboardButton("◀️ Back", callback_data="settings")]]
...
async def s_max(update, context):
    q = update.callback_query; await q.answer()
    d = init(context)
    await q.edit_message_text(f"💰 Max Amount ထည့်ပါ (MMK):\nCurrent: {d['max']}",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Cancel", callback_data="settings")]]))
    return S_MAX
...
async def s_min(update, context):
    q = update.callback_query; await q.answer()
    d = init(context)
    await q.edit_message_text(f"💰 Min Amount ထည့်ပါ (MMK):\nCurrent: {d['min']}",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Cancel", callback_data="settings")]]))
    return S_MIN
...
async def s_target(update, context):
    q = update.callback_query; await q.answer()
    d = init(context)
    await q.edit_message_text(f"🎯 Target Price ထည့်ပါ (MMK):\nCurrent: {d['target']}",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Cancel", callback_data="settings")]]))
    return S_TARGET
...
async def s_orders(update, context):
    q = update.callback_query; await q.answer()
    d = init(context)
    await q.edit_message_text(f"🔢 Max Orders ထည့်ပါ:\nCurrent: {d['orders']}",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Cancel", callback_data="settings")]]))
    return S_ORDERS
...
async def cb_history(update, context):
    q = update.callback_query; await q.answer()
    d = init(context)
    text = f"📋 Order History\n\nTotal: {d['total']}\nThis Month: {d['monthly']}\nVolume: {d['volume']} MMK"
    kb = [[InlineKeyboardButton("◀️ Back", callback_data="stats")]]
    await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb))
    return MAIN
    ...
    conv = ConversationHandler(
        ...
        states={
            MAIN: [
                ...
                CallbackQueryHandler(cb_history, pattern="^history$"),
            ],
            SETTINGS: [
